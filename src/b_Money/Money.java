...
public class Money implements Comparable {
	 ...
	 * e.g. "10.5 SEK". Recall that we represent decimal numbers with integers. This
	 * means that the "10.5 SEK" mentioned above is actually represented as the
	 * integer 1050
	 * 
	...
	public String toString() {
		...
		return String.format(Locale.US, "%.2f %s", amountDouble, currency.getName());
	}
	...
	public Boolean equals(Money other) {
		...
		return Math.abs(this.universalValue() - other.universalValue()) < epsilon;
	}
	 ...
	 *         added value of the two. (Remember to convert the other Money before
	 *         adding the amounts)
	 */
	public Money add(Money other) {
		if (this.currency.equals(other.getCurrency())) {
			...
			int amountInThisCurrency = this.currency.valueInThisCurrency(sum, this.currency);
			return new Money(amountInThisCurrency, this.currency);
		}
	}
	 ...
	 * @return A new Money with the same Currency as this Money, representing the
	 *         subtracted value. (Again, remember converting the value of the other
	...
	public Money sub(Money other) {
		if (this.currency.equals(other.getCurrency())) {
			...
			int amountInThisCurrency = currency.valueInThisCurrency(difference, currency);
			return new Money(amountInThisCurrency, this.currency);
		}
	}
	...
	/**
	 * Compare two Monies. compareTo is required because the class implements the
	 * Comparable interface. (Remember the universalValue method, and that Integers
	 * already implement Comparable). Also, since compareTo must take an Object, you
	 * will have to explicitly downcast it to a Money.
	 * 
